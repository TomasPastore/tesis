...
from db_parsing import (get_granularity, all_loc_names, preference_locs,
                        load_patients, EVENT_TYPES, HFO_TYPES, WHOLE_BRAIN_L0C)
...
# TODO review and remove this version, localized version is more general
...
# 3 ii
# Note: This takes into account ANY kind of coupling HFO (slow, delta, etc),
# though that parameter can be adjusted by load_patients function of db_parsing
...
def pse_hfo_rate_auc_relation(elec_collection, evt_collection):
    data_by_loc = evt_rate_soz_pred_baseline_localized(elec_collection,
                                             ...
                                             locations={
                                                 ...
                                                 in [2, 3, 5]},
                                             saving_dir=
                                             FIG_SAVE_PATH[3]['ii']['dir'],
                                             plot_rocs=False)

    graphics.plot_pse_hfo_rate_auc_table(data_by_loc, str(Path(FIG_SAVE_PATH[3][
                                                                   'ii']['dir'],
                                                               'table')))

    graphics.plot_co_pse_auc(data_by_loc, str(Path(FIG_SAVE_PATH[3][
                                                       'ii']['dir'],
                                                   'scatter')))


# 3.iii
# TODO pAUC instead of AUC
# FIXME make legend boxes in figures adaptive to fig size
# Returns the data for the ml and the data to plot the ROC baselines
def evt_rate_soz_pred_baseline_localized(elec_collection,
    ...
    # patients_by_loc = None
    # Now we load all and then solve inconsistencies in RAM
    ...
    for granularity, locs in locations.items():
        ...
        for loc_name in locs:
            ...
            file_saving_path = str(Path(saving_dir,
                                        ...
                                        loc_name.replace(' ', '_'),
                                        loc_name.replace(' ', '_') + '_sleep'))
            # Create parents dirs if they don't exist
            ...
            with open(info_saving_path, "w+") as file:
                ...
                for event_type_names in evt_types_to_cmp:
                    ...
                    min_pat_with_epilepsy_in_location = 3  # Suggested by Shennan
                    if loc_info['patient_count'] >= min_pat_count_in_location \
                        ...
                        print('Region and type excluded due to lack of '
                              'data --> {0} {1}'.format(loc_name, type_group_name))
        if plot_rocs:
            graphics.event_rate_by_loc(event_type_data_by_loc,
                                       metrics=['pse', 'pnee', 'auc'],
