...
def predict_folds(folds, target_patients, hfo_type_name, models):
    for model_name in models:
        ...
        for fold in folds:
            clf_preds, clf_probs = model_func(fold['train_features'], fold['train_labels'], fold['test_features'],
...
def phfo_predictor_v0(hfo_type_name, patients_dic, target=None, models=['XGBoost']):
    if target is None:
        target = ['model_pat', 'validation_pat']
    model_patients, target_patients, test_partition = build_patient_sets(target, hfo_type_name, patients_dic)
    folds = build_folds(hfo_type_name, model_patients, target_patients, test_partition)
    for model_name in models:
        predict_folds(folds, target_patients, hfo_type_name, models=models)
