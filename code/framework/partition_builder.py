...
def pull_apart_validation_set(patients_dic, hfo_type_name):
    ...
    print('Validation patient names {0}'.format(partition_names[0]))
...
def analize_balance(train_labels):
    ...
    for l in train_labels:
        ...
        if l:
            ...
            negative_class += 1
    print('Percentages in analize partition: Positive: {0} Negative: {1}. Count: {2}'.format(
        round(positive_class / tot, 2)))
...
def build_folds(hfo_type_name, model_patients, target_patients, test_partition):
    ...
    for p_names in test_partition:
        ...
        test_patients = [p for p in target_patients if p.id in p_names]
