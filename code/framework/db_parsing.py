...
def load_patients(electrodes_collection, evt_collection, intraop, loc_granularity, locations,
    ...
    for loc_name in locations:
        ...
        elec_filter, evt_filter = query_filters(intraop, event_type_names, loc, loc_name,
                                                ...
                                                allow_empty_loc=allow_empty_loc)
        ...
        patients_by_loc[loc_name] = parse_patients(elec_cursor, hfo_cursor, event_type_names, models_to_run,
...
def parse_patients(electrodes_cursor, hfo_cursor, event_type_names, models_to_run, rm_xyz_null, rm_loc_empty):
    print('Parsing db patients...')
    patients_dic = dict()
    parse_electrodes(patients_dic, electrodes_cursor, event_type_names)
    parse_events(patients_dic, hfo_cursor, event_type_names, models_to_run, remove_xyz_null=rm_xyz_null,
...
def parse_electrodes(patients, elec_cursor, event_type_names):
    for e in elec_cursor:
        ...
        loc1, loc2, loc3, loc4, loc5 = parse_loc(e, 1), parse_loc(e, 2), parse_loc(e, 3), parse_loc(e, 4), parse_loc(e,
                                                                                                                     5)
...
# Modifies patients
def parse_events(patients, event_collection, event_type_names, models_to_run, remove_xyz_null=False,
    ...
    for evt in event_collection:
        ...
        loc1, loc2, loc3, loc4, loc5 = parse_loc(evt, 1), parse_loc(evt, 2), parse_loc(evt, 3), parse_loc(evt,
                                                                                                          4), parse_loc(
            evt, 5)
        ...
        for type in e.events.keys():
            for evt in e.events[type]:
                if evt.info['soz'] != e.soz:
                    raise RuntimeError('Found soz inconsistency in electrode/event')
...
def fix_elec_evt_xyz_consistency(patient):
    for e in patient.electrodes:
        for type in e.events.keys():
            for evt in e.events[type]:
                for attr in ['x', 'y', 'z']:
                    if (getattr(e, attr) is not None) and (evt.info[attr] is None):
            ...
            for evt in e.events[type]:
                for attr in ['x', 'y', 'z']:
                    if getattr(e, attr) != evt.info[attr]:
                        raise RuntimeError('Found x,y,z inconsistency in electrode/event')
...
def check_elec_xyz_not_null(patient):
    for elec in patient.electrodes:
        assert (all([getattr(elec, attr) is not None for attr in ['x', 'y', 'z']]))
        # if any([getattr(elec, attr) is None for attr in ['x', 'y', 'z']]):
        #    elec.print()
        ...
        for type in elec.events.keys():
            for evt in elec.events[type]:
                for attr in ['loc2', 'loc3', 'loc5']:
                    ...
                    if getattr(elec, attr) != 'empty' and evt.info[attr] == 'empty':
                        # Fix inconsistency
                        evt.info[attr] = getattr(elec, attr)
                        # Log inconsistency
                        if getattr(elec, attr) in evts_with_wrong_empty_loc.keys():
                    ...
                    elif getattr(elec, attr) != evt.info[attr] and evt.info[attr] != 'empty':
                        raise RuntimeError('loc not empty disagreement among events for the same (pat_id, electrode)')
            ...
            for evt in elec.events[type]:
                for attr in ['loc2', 'loc3', 'loc5']:
                    if getattr(elec, attr) != evt.info[attr]:
                        ...
                        raise RuntimeError('Found loc inconsistency in electrode/event')


def remove_elec_loc_empty(patient):
    to_remove = []
    for elec in patient.electrodes:
        if any([getattr(elec, attr) == 'empty' for attr in ['loc2', 'loc3', 'loc5']]):
            to_remove.append(elec)
    patient.remove_electrodes(to_remove)
...
def check_elec_loc_not_empty(patient):
    for elec in patient.electrodes:
        assert (all([getattr(elec, attr) != 'empty' for attr in ['loc2', 'loc3', 'loc5']]))
...
def parse_coord(param):  # -1 Represents empty, consider filtering
    parsed_coord = None if (isinstance(param, list) or param == '-1') else float(param)
...
def decode_type_name(type_id):  # Returns the event name string from the db code number
